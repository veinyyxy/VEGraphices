...
bool InterpolationTriangle::Execute( GISPolygon *pGISPolygon)
{
	CBlockTIN pBlockTIN;
	CBlockContour pBlockContour;
	ClearArray();
...
bool InterpolationTriangle::GetGISPolyFromTIN(CBlockTIN* pBlockTIN)
{
	...
	m_pPolygonTin = new GISPolygon[pBlockTIN->lTriangleCount];
	for (i=0; i < pBlockTIN->lTriangleCount; i++)
	{
		m_pPolygonTin->Mode(GL_TRIANGLES);
...
bool InterpolationTriangle::GetContourFromGISPoly( GISPolygon *pGISPolygon, CBlockContour *pBlockContour )
{
